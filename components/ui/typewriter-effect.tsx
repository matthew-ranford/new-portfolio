...
export const TypewriterEffect = ({
            ...
            <div key={`word-${idx}`} className="inline">
              {word.text.map((char, index) => (
                <motion.span
                  ...
                  key={`char-${index}`}
                  className={cn(
                    `opacity-0 hidden dark:text-zinc-300 text-stone-800 dark:drop-shadow-[4px_4px_0px_#004aad] drop-shadow-[4px_4px_0px_#66045f]`,
                    word.className
                  )}
                >
                  {char}
  ...
  return (
    <div
      className={cn(
        'text-7xl xl:text-8xl 2xl:text-9xl 6xl:text-[16rem]',
        className
      )}
    >
      {renderWords()}
